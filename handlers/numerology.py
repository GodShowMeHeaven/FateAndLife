...
def validate_date(birth_date: str) -> bool:
    """Проверяет, что дата в формате ДД.ММ.ГГГГ"""
    return bool(re.match(r"^\d{2}\.\d{2}\.\d{4}$", birth_date))

async def numerology(update: Update, context: CallbackContext) -> None:
    """Обрабатывает команду /numerology"""
    if not context.args:
        await update.message.reply_text("🔢 Введите вашу дату рождения в формате: `/numerology ДД.ММ.ГГГГ`")
        return
    ...
    try:
        life_path_number = calculate_life_path_number(birth_date)
        interpretation = ask_openai(f"Напиши эзотерическое толкование числа судьбы {life_path_number}.
    Опиши ключевые качества личности, предназначение и кармический смысл этого числа.
    Добавь мистическую символику и советы по гармонизации энергии."
)
        await update.message.reply_text(f"🔢 **Ваше число судьбы: {life_path_number}**\n\n{interpretation}")

    except Exception as e:
        logger.error(f"Ошибка в numerology: {e}")
